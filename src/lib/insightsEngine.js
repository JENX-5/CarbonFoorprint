...
export function generateInsights(values, byCategoryAnnual) {
  ...
  const annualTotal = Data.CATEGORY_ORDER.reduce(
    (sum, k) => sum + byCategoryAnnual[k],
    0,
  );

  const topCategory = Data.CATEGORY_ORDER.reduce(
    (best, key) =>
      byCategoryAnnual[key] > byCategoryAnnual[best] ? key : best,
    Data.CATEGORY_ORDER[0],
  );

  const topShare =
    annualTotal > 0 ? (byCategoryAnnual[topCategory] / annualTotal) * 100 : 0;

  const candidates = [];

  const transportSavings = byCategoryAnnual.transportation * 0.2;
  candidates.push({
  ...
  let dietSavings = 0;
  let dietText =
    "Your diet is already in the lowest-impact tiers, so no further rule-based saving was identified here.";
  if (values.dietType !== "vegetarian" && values.dietType !== "vegan") {
    ...
    dietText =
      "Shifting toward a vegetarian pattern would lower the daily emission factor behind your meals.";
  }
  candidates.push({
    category: "diet",
    savingsKg: dietSavings,
    text: dietText,
    simulatorPreset: { dietTarget: "vegetarian" },
  });

  const recycledAt90Factor =
    f.waste.landfill * 0.1 + f.waste.recycledOrComposted * 0.9;
  ...
  const recommendations = candidates
    .filter((c) => c.savingsKg > 0.5)
    .slice(0, 3);
  const totalPotentialSavings = recommendations.reduce(
    (sum, c) => sum + c.savingsKg,
    0,
  );
